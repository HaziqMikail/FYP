...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $digital_email = $_POST['email'] ?? null;
    $digital_password = $_POST['password'] ?? null;
    ...
    $stmt = $conn->prepare("INSERT INTO transactions 
        (transaction_id, seller_id, item_name, amount, marketplace_link, product_type, digital_email, digital_password, image_path) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sisdsssss", $transaction_id, $seller_id, $item_name, $amount, $marketplace_link, $product_type, $digital_email, $digital_password, $image_path);
...
<div class="main-content fade-in">
  <h1 style="margin-bottom: 20px;">Create Product</h1>
  <?php if ($message): ?>
...
<script>
  function toggleDigitalFields() {
    ...
    digitalFields.style.display = type === 'digital' ? 'block' : 'none';
  }
</script>
